            ...
            <div class="detail-info">
              ...
              <div>
                <!-- <el-row :gutter="20" v-if="show">
                  <el-col :span="16">
                    <el-input type="text" placeholder="请输入验证码答案" style="width:250px"/>
                  </el-col>
                  <el-col :span="8">
                    <img
                      src="http://127.0.0.1:8888/api/verifyCode"
                ...
                <el-button
                  id="book-btn"
                  :type="isBookButtonDisabled ? 'info' : 'primary'"
                  v-if="showButtonType == 0"
                  ref="book"
                ...
                <router-link to="/home/order/list">
                  <el-button type="primary" v-if="showButtonType == 1" id="trun-btn">
                    <span>查看订单</span>
                  </el-button>
                </router-link>
              ...
              <div class>
                <br>
                <p>
                  课程编号: {{course.courseNo}}
                  <br>
                </p>
                <p>
                  课程开始时间: {{course.startTime | date-format}}
                  <br>
                </p>
                <p>
                  课程结束时间: {{course.endTime | date-format}}
                  <br>
                  <br>
                </p>
...
export default {
  ...
  created() {
    ...
    var self = this;
    self.axios
      .get("/api/courseDetail/" + self.$route.params.courseNo)
  ...
  methods: {
    book() {
      ...
      var self = this;
      self.axios
        .get("/api/getPath/" + self.$route.params.courseNo)
        .then(function(response) {
          var path = response.data;
          var courseNo = self.$route.params.courseNo;
          self.axios
            .get(`/api/${path}/seckill/${courseNo}`)
            .then(function(response) {
    ...
    getResult(response) {
      ...
      switch (code) {
        ...
        case 500100:
          ...
          self.load = true;
          self.axios
            .get("/api/seckillResult/" + self.$route.params.courseNo)
            .then(function(response) {
              self.getResult(response);
            })
            .catch(function(error) {
